...
def estimate_uncertainty(method, X_test, gbrt, X_train, y_train, rng, tau=0.75, m=100):
    """
    ...
    Input
        ...
        tau: float, normalized affinity threshold ("flex_pgbm" method).
        m: int, no. nearest neighbors to use ("flex_pgbm" method).
    ...
    if method == 'flex_pgbm':
        ...
        pred, ci = fpgbm.pred_dist(X_test, tau=tau, m=m)
...
def main(args):
    ...
    for i, n in enumerate(n_list):
        ...
        gbrt = util.get_model(tree_type=args.tree_type, objective=objective, random_state=args.rs)
        gbrt.set_params(**hp)
        gbrt = gbrt.fit(X_train, y_train)

        # util.eval_pred(objective, gbrt, X_test, y_test, None, prefix='Test')
        ...
        pred, ci = estimate_uncertainty(args.method, X_test, gbrt, X_train, y_train, rng,
                                        tau=args.tau, m=args.m)

        y_pred = gbrt.predict(X_test)

        print(pred, ci, y_test)

        # TEMP: avoid 0 ci
        ci = ci + 1e-8

        crps = np.mean([ps.crps_gaussian(y_test[i], mu=pred[i], sig=ci[i]) for i in range(len(X_test))])
        nll = np.mean([-stats.norm.logpdf(y_test[i], loc=pred[i], scale=ci[i]) for i in range(len(X_test))])
        rmse = mean_squared_error(y_test, pred, squared=False)
        rmse_og = mean_squared_error(y_test, y_pred, squared=False)
        print(f'CRPS: {crps:.5f}, NLL: {nll:.5f}, RMSE: {rmse:.5f}, RMSE (OG): {rmse_og:.5f}')

        nlls = [-stats.norm.logpdf(y_test[i], loc=pred[i], scale=ci[i]) for i in range(len(X_test))]
        print(f'\nNLLs:\n{nlls}')
