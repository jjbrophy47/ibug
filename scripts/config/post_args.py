...
def get_explainer_args(cmd=None):
    ...
    if cmd is None:
        cmd = configargparse.ArgParser(config_file_parser_class=configargparse.YAMLConfigFileParser)
    cmd.add('--method', type=str, nargs='+', default=['random', 'vog_asc', 'vog_desc'])
...
def get_vog_args():
    """
    Add arguments specific to "VOG" postprocessing.

    Return ArgParser object.
    """
    cmd = get_general_args()
    cmd = get_explainer_args(cmd)
    cmd.add('--in_dir', type=str, default='output/experiments/vog/')
    cmd.add('--out_dir', type=str, default='output/postprocess/vog/')
    return cmd


def get_compression_args():
    """
    Add arguments specific to "compression" postprocessing.

    Return ArgParser object.
    """
    cmd = get_general_args()
    cmd = get_explainer_args(cmd)
    cmd.add('--in_dir', type=str, default='output/experiments/compression/')
    cmd.add('--out_dir', type=str, default='output/postprocess/compression/')
    return cmd
