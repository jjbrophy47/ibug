...
def main(n_splits=10, random_state=1):
    ...
    columns = ['likes', 'page_check', 'page_talk', 'page_cat']
    columns += [f'X{i}' for i in range(5, 30)]
    columns += ['CC1', 'CC2', 'CC3', 'CC4', 'CC5', 'base_time', 'post_len',
                'share_count', 'promo_status', 'h_local']
    columns += [f'post_{day}' for day in days]
    columns += [f'base_{day}' for day in days]
    columns += ['num_comments']

    # retrieve dataset
    start = time.time()
    df = pd.read_csv('Dataset/Training/Features_Variant_1.csv', header=None, names=columns)
    ...
    features = {}
    features['label'] = ['num_comments']
    features['numeric'] = [c for c in columns if c not in ['page_cat', 'num_comments']]
